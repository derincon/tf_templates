...
resource "oci_core_instance" "wls_instance" {
  ...
  fault_domain = (var.ad_number>=0 || !var.use_regional_subnet)?lookup(data.oci_identity_fault_domains.wls_fault_domains.fault_domains[(count.index + 1) % local.num_fault_domains], "name"):""
  timeouts {
