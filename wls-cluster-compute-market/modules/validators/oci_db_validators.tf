...
locals {
  ...
  missing_ocidb_dns_subnet_cidr = (local.is_vcn_peering && var.ocidb_dns_subnet_cidr=="" && !var.disable_infra_db_vcn_peering)
  ...
  #ocidb password OCID validation, oci_db_password_ocid is mapped to db_password
  invalid_oci_db_password_msg  = "WLSC-ERROR: The value for DB System Admin Password [oci_db_password_ocid] is not valid. The value must begin with ocid1 followed by resource type, e.g. ocid1.vaultsecret."
  validate_oci_db_password      = var.is_oci_db && var.db_password != "" ? length(regexall("^ocid1.vaultsecret.", var.db_password)) > 0 ? null : local.validators_msg_map[local.invalid_oci_db_password_msg] : null
