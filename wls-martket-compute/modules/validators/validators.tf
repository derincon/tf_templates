...
locals {
  ...
  invalid_service_name_prefix = (length(var.service_name_prefix)>16) || (length(var.service_name_prefix)<1) || (length(replace(substr(var.service_name_prefix, 0, 1), "/[0-9]/", ""))==0) || length(var.service_name_prefix) != length(var.original_service_name)
  invalid_ocpu_count          = (var.wls_ocpu_count < 1) || (var.wls_ocpu_count > 64)
  invalid_vm_count            = (var.numVMInstances <1) || (var.numVMInstances > var.wls_node_count_limit)
  ...
  #lb2 check
  duplicate_lb2_subnet_cidr_with_private_subnet_cidr = (var.add_load_balancer) && ( var.use_regional_subnet == false ) && ((!var.assign_public_ip) && var.bastion_subnet_id=="") && (local.has_mgmt_subnet_cidr) && (local.has_lb_subnet_2_cidr) && (var.lb_subnet_2_cidr == var.bastion_subnet_cidr)
  ...
  is_vcn_peering                  = ((var.vcn_name !="" && var.ocidb_existing_vcn_id != "") || (var.existing_vcn_id != "" && var.ocidb_existing_vcn_id != "" && var.existing_vcn_id != var.ocidb_existing_vcn_id) && !var.disable_infra_db_vcn_peering)
  ...
  missing_wls_dns_subnet_info = local.is_vcn_peering && (var.wls_dns_subnet_cidr=="") && !var.disable_infra_db_vcn_peering

  peering_not_enabled = local.is_vcn_peering && !var.use_local_vcn_peering && var.disable_infra_db_vcn_peering
  ...
  invalid_app_db_selection = var.wls_edition == "SE" && var.configure_app_db
  invalid_ocidb_selection  = var.wls_version == "11.1.1.7" && var.ocidb_version != "" && (substr(var.ocidb_version, 0, 2) == "19" || substr(var.ocidb_version, 0, 2) == "18")
  invalid_app_ocidb_selection  = var.wls_version == "11.1.1.7" && var.app_ocidb_version != "" && (substr(var.app_ocidb_version, 0, 2) == "19" || substr(var.app_ocidb_version, 0, 2) == "18")
  ...
  numVMInstances_msg = "WLSC-ERROR: The value for wls_node_count=[${var.numVMInstances}] is not valid. The permissible value cannot exceed the value wls_node_count_limit=[${var.wls_node_count_limit}]."
  ...
  #wls admin password OCID validations
  invalid_wls_admin_password_msg  = "WLSC-ERROR: The value for WebLogic Admin Password [wls_admin_password_ocid] is not valid. The value must begin with ocid1 followed by resource type, e.g. ocid1.vaultsecret."
  ...
  unsupported_app_db_msg = "WLSC-ERROR: Application Database configuration is not supported for SE edition."
  validate_unsupported_app_db = local.invalid_app_db_selection ? local.validators_msg_map[local.unsupported_app_db_msg] : ""
