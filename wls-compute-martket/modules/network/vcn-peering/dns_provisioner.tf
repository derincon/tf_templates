...
data "oci_core_vnic_attachments" "appdb_vmnics" {
  ...
  instance_id         = oci_core_instance.appdb_dns_vm[0].id
}
...
data "oci_core_vnic" "appdb_vnic" {
  count   = var.appdb_vcn_peering?1:0
  vnic_id = lookup(data.oci_core_vnic_attachments.appdb_vmnics[0].vnic_attachments[0],"vnic_id")
}
...
data "template_file" "generate_wls_dnsmasq_conf" {
  ...
  vars = {
    ocidb_dns_private_ip = var.is_vcn_peering ? data.oci_core_vnic.ocidb_vnic[count.index].private_ip_address : ""
    ocidb_zone           = var.is_vcn_peering ? "${lookup(data.oci_core_vcns.ocidb_vcn[0].virtual_networks[0], "dns_label")}.oraclevcn.com" : ""
    appdb_dns_private_ip = var.appdb_vcn_peering ? data.oci_core_vnic.appdb_vnic[count.index].private_ip_address : ""
    appdb_zone           = var.appdb_vcn_peering ? "${lookup(data.oci_core_vcns.appdb_vcn[0].virtual_networks[0], "dns_label")}.oraclevcn.com" : ""
  }
}
...
resource "null_resource" "appdb-dns-configure-dnsmasq-newvcn" {
  count = var.appdb_vcn_peering && var.wls_vcn_name != ""? 1:0

  connection {
    type        = "ssh"
    user        = "opc"
    private_key = tls_private_key.dns_opc_key[0].private_key_pem
    host        = data.oci_core_vnic.appdb_vnic[0].public_ip_address
    timeout     = "30m"
  }
...
resource "null_resource" "appdb-dns-configure-dnsmasq-existingvcn" {
  count = var.appdb_vcn_peering && var.wls_vcn_name == ""? 1:0
