...
data "template_file" "ad_number" {
  ...
  template =  (var.instance_shape=="VM.Standard.E3.Flex" || (tonumber(lookup(data.oci_limits_limit_values.compute_shape_service_limits[count.index].limit_values[0], "value")) > 0))?lookup(data.oci_identity_availability_domains.ADs.availability_domains[ad_number], "name"):""
}
